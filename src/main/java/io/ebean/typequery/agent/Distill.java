...
class Distill {
	 ...
	 */
	static DetectQueryBean convertDetectQueryBean(Collection<String> packages) {
	...
	static DetectDescriptorBean convertDetectDescriptorBean(Collection<String> packages) {

		String[] asArray = packages.toArray(new String[packages.size()]);
		for (int i = 0; i < asArray.length; i++) {
			asArray[i] = convert(asArray[i]);
		}
		return new DetectDescriptorBean(asArray);
	}
