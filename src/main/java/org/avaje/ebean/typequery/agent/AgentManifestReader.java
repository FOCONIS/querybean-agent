...
public class AgentManifestReader {
	...
	public static Set<String> readQueryMf(ClassLoader classLoader, Set<String> initialPackages) {
		try {
	...
	public static Set<String> readDescriptorMf(ClassLoader classLoader, Set<String> initialPackages) {
		try {
			return new AgentManifestReader(initialPackages)
					.readManifests(classLoader, "META-INF/ebean-descriptor.mf")
					.getPackages();

		} catch (IOException e) {
			// log to standard error and return empty
			System.err.println("QueryBean Agent: error reading META-INF/ebean-descriptor.mf manifest resources");
			e.printStackTrace();
			return new HashSet<>();
		}
	}
